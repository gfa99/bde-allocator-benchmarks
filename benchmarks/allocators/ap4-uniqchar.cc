...
#if defined(USE_ALLOC) || defined(USE_COUNT) || defined(USE_LOCAL) || defined(USE_WINKOUT)
std::size_t unique_chars(const std::string& s)
{
#if defined(USE_LOCAL) || defined(USE_WINKOUT)
    // Size determined from impirical measurements
    bdlma::LocalSequentialAllocator<2048> alloc;
#elif defined(USE_COUNT)
    bdlma::SequentialAllocator seq_alloc;
    bdlma::CountingAllocator   alloc(&seq_alloc);
#else
    ...
    std::cout << "In use = " << alloc.numBytesInUse()
              << ", Total = " << alloc.numBytesTotal() << std::endl;
#endif
    return uniq.size();
    // Wink out of uniq would happen here
}

#else // If not USE_ALLOC or USE_WINKOUT
...
{
#if USE_COUNT
    int reps = 1;  // Only one iteration of counting is really needed
#else
    int reps = 1000000;
    if (argc > 1) {
